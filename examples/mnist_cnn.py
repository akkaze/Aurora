...
def measure_accuracy(activation, data, batch_size=32, use_gpu=False):
    ...
    for i in range(0, max_val, batch_size):
        ...
        prediction[start:end] = xp.argmax(prob_val, axis=1)

    correct = xp.sum(xp.equal(y_val, prediction))
...
if __name__ == "__main__":
    ...
    loss, W1, b1, W2, b2, W3, b3, W4, b4, logits = build_network(
        images, labels, batch_size=128
    )
    ...
    val_acc = measure_accuracy(
        logits, data.validation(), batch_size=128, use_gpu=use_gpu
    )
